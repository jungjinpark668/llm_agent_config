...
4. **Zero-trust evidence rule.** Codex's unaided intuition is trusted 0%. Never guess, never assume. Every logical step must rest on existing code, a golden reference, or proven data — and the delivery must cite which ("golden X says Y, therefore Z"). If no golden reference exists for a claim, STOP and ask the user to confirm the mental model. No skipping this.
...
7. **Compaction rule.** Auto-compact does not trigger reliably. At 40–45% context usage: run /checkpoint, then compact. After ANY compaction or context reset, re-read this AGENTS.md and refresh the doctrine before continuing.

## Identity & Notes Vault

The Obsidian vault is Codex's persistent memory. When Codex learns something, makes a connection, solves a problem, or builds context across sessions — it lives here.
...
1. **Future value** — Would a future Codex instance genuinely benefit? (not just "nice to have")
